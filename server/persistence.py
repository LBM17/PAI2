...
def add_user(username: str, plain_password: str) -> Tuple[bool, str]:
    ...
    if cur.fetchone():
        c.close(); return False, "Usuario ya existe"
    import secrets, time
    salt = secrets.token_hex(16)
    ...
    c.close(); return True, "Usuario registrado"
...
def store_nonce(nonce: str, ts: int):
    ...
    with c:
        c.execute(
            "INSERT OR IGNORE INTO nonces_seen(nonce,ts) VALUES (?,?)",
            (nonce, ts)
        )
    c.close()

def add_tx(username: str, src: str, dst: str, amount: float, nonce: str, ts: int, mac: str):
    c = conn()
    with c:
        c.execute(
